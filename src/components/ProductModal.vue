      ...
      <div class="modal-content border-0">
        <div class="modal-header bg-dark text-white">
          <h5 id="productModalLabel" class="modal-title">
            <span v-if="!tempData.id">新增產品</span>
            <span v-else>編輯: {{ currentProduct.title }}</span>
          </h5>
          <button
            type="button"
...
export default {
  ...
  watch: {
    currentProduct() {
      this.tempData = { ...this.currentProduct }
    }
  },

  methods: {
    sendData() {
      this.$emit('seed-data', this.tempData)
    }
  }
}
</script>
